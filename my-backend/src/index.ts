	...
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		try {
			...
			if (url.pathname.startsWith('/game/')) {
				...
				return (await stub.fetch(request as any)) as unknown as Response;
			}
...
async function handleTournamentRequest(
	...
	const response = (await stub.fetch(forwardRequest as any)) as unknown as Response;
...
async function handleTournamentCreation(
	...
	return new Response(JSON.stringify({ id: tournamentId }), {
		status: 201,
		headers: { ...corsHeaders, 'Content-Type': 'application/json' },
	});
}
